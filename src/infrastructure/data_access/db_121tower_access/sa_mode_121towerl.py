...
from src.infrastructure.data_access.db_121tower_access.base import Base
from src.infrastructure.data_access.db_121tower_access.constants import (
    ChassisType,
    EquipmentType,
    EventType,
    ShipmentStatus,
...
class TimestampMixin(CreatedTimestampMixin):
    updated_dt = Column(DateTime, server_default=utcnow(), onupdate=utcnow(), nullable=False)
...
class SALocation(TimestampMixin, SAExternalRefMixin, Base):
    ...
    @classmethod
    async def get_location_by_id(cls, orm_client: AlchemyBase, id: int) -> Self | None:
        try:
