...
class StopsImpl(StopsRepositoryABC):
    ...
    async def save_and_sync_stops(self, list_of_shipments: List[Shipment]):
        ...
        assert rows, f"don't not found stops to sync at {datetime.now()}"
        ...
        assert row_next_id, f"Don't not found next Id for ''Stops WH'' at {datetime.now()}"
        ...
        for row_query in rows:
            ...
            if unique_key_event not in unique_event:
                ...
                if current_event:
                    ...
                    current_event.stop.duplicate = is_duplicate
                    current_event.stop.duplicate_counter = duplicate_stop_counter.get(unique_key_stopid) if duplicate_stop_counter else None
